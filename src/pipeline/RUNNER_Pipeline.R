...
source("./utils.R")
source("./algoRICurves.R")
source("./plotRICurve.R")
...
inputData <- read.table(file = "PATH-TO-FILE", header = T, sep ="\t",  stringsAsFactors =F)
...
bootstrapRes <- runPipelineCIs(inputData = inputData, pointEstGamlss = pointEstGamlss, NBootstrap = NBootstrap, 
...
allRes <- estimateCIs(pointEst = pointEstGamlss, estBS = bootstrapRes, covarValue = covarValue, withData = TRUE, RIperc = RIperc, CIprop = 0.95, onlyCI =FALSE, 
...
# plot estimates on split x-axis  with confidence intervals(first 30 days, first year, 1-18) (only useful if you have pediatric data) 
png(filename = file.path(outputDir, paste0(filename, "_", "CIs.png")), width = 1200, height = 700, res = 100)
plotRICurveEstimatesSplitAxis(allRes = allRes, withCI = TRUE, RIperc = RIperc)
dev.off()


cat(paste0(Sys.time(), ": Pipeline completed. Results can be found in ", outputDir, ". \n"))
