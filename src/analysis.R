...
# ==============================================================================
# Analysis 1: Out-of-RI flagging rates (Table 3)
# Analysis 2: Guven et al. RI transferability (CLSI EP28-A3c)
# Analysis 3: Age-specific out-of-RI flagging curves (Figure)
# Analysis 5: International comparison table
# Analysis 5b: Age partitioning justification (Harris-Boyd)
# Analysis 6: Bootstrap CI for out-of-RI flagging rates
# ==============================================================================
...
# "Outside" = below lower OR above upper limit. Note: this is the flagging rate
# in the mixed routine cohort, not a false-positive rate in a healthy cohort.

message("[ANALYSIS 1] Computing out-of-RI flagging rates by RI source...")
...
# 5) Direct (a posteriori): same partitions as refineR
...
message("\n=== TABLE 3: Overall Out-of-RI Flagging Rates (%) ===")
...
message("[ANALYSIS 3] Generating age-specific out-of-RI flagging curves...")
...
# This study's own rows are taken from ri_comparison in common.R rather than
# written out again, so the comparison table cannot fall behind the limits the
# rest of the analysis uses.
own_rows <- ri_comparison |>
  transmute(
    study     = ifelse(method == "refineR", "This study (refineR)", "This study (Direct)"),
    year      = 2026,
    country   = "Turkey",
    analyzer  = "Cobas t511/t711",
    method    = ifelse(method == "refineR", "Indirect (refineR)", "Direct (a posteriori)"),
    n, age_group, test, lower, upper,
    percentiles = "2.5-97.5",
    unit      = ifelse(test == "Fibrinogen", "g/L", "s")
  ) |>
  arrange(match(study, c("This study (refineR)", "This study (Direct)")),
          match(test, c("PT", "aPTT", "Fibrinogen")), age_group)

intl_comparison <- bind_rows(own_rows, tribble(
  ...
  "Guven et al.", 2026, "Turkey", "Cobas t511", "Direct", 1065, "0-18 y", "PT", 7.8, 10.7, "2.5-97.5", "s",
  "Guven et al.", 2026, "Turkey", "Cobas t511", "Direct", 1044, "0-18 y", "aPTT", 24.3, 36.2, "2.5-97.5", "s",

  # --- Luo et al. 2026 (Sysmex CN-6000, Chinese children, Table 1) ---
  # L. Luo, Q. Duan, J. Huang, D. Peng, Y. Kuang, Y. Wang, L.R. Brandao, X. Fu,
  # Establishment of age-specific reference intervals for 16 coagulation
  # parameters in Chinese children using Sysmex CN-6000 analyzer: a
  # cross-sectional study, Health Sci. Rep. 9 (2026) e72023.
  # https://doi.org/10.1002/hsr2.72023
  # All limits and sample sizes below were checked against the paper's Table 1
  # on 2026-08-15 and match exactly.
  ...
  "Manufacturer (Roche)", NA, NA, "Cobas t511/t711", "Package insert", NA, "Adult (18-50 y)", "PT", 8.4, 10.6, NA, "s",
  "Manufacturer (Roche)", NA, NA, "Cobas t511/t711", "Package insert", NA, "Adult (18-50 y)", "aPTT", 23.6, 30.6, NA, "s",
  "Manufacturer (Roche)", NA, NA, "Cobas t511/t711", "Package insert", NA, "Adult (18-50 y)", "Fibrinogen", 1.93, 4.12, NA, "g/L"
))

# The manufacturer rows must agree with ri_manufacturer, and this study's rows
# with ri_comparison, both defined in common.R.
local({
  m <- intl_comparison |> filter(study == "Manufacturer (Roche)") |>
    select(test, lower, upper) |> arrange(test)
  stopifnot(isTRUE(all.equal(as.data.frame(m),
    as.data.frame(arrange(select(ri_manufacturer, test, lower, upper), test)))))
  own <- intl_comparison |> filter(grepl("^This study", study)) |> nrow()
  stopifnot(own == nrow(ri_comparison))
})
...
# Computed on all_data, the full canonical set, which is the basis the
# manuscript and the Supplementary Statistical Methods report the statistic on.
aptt_young <- all_data |> filter(test == "aPTT", age_int < 12)
aptt_old   <- all_data |> filter(test == "aPTT", age_int >= 12)
...
pt_young <- all_data |> filter(test == "PT", age_int < 12)
pt_old   <- all_data |> filter(test == "PT", age_int >= 12)
...
# ANALYSIS 6: BOOTSTRAP CI FOR OUT-OF-RI FLAGGING RATES
# ==============================================================================
# For each test: bootstrap (B=1000) the overall flagging rate per RI source.
# Report 95% CI.

message("[ANALYSIS 6] Bootstrap CI for out-of-RI flagging rates...")
...
boot_direct_aptt <- function() {
  d <- all_data_3sd |> filter(test == "aPTT")
  ri_y <- ri_direct |> filter(test == "aPTT", age_group == "1-12")
  ri_o <- ri_direct |> filter(test == "aPTT", age_group == "12-18")
  d <- d |> mutate(lo = ifelse(age_int < 12, ri_y$lower, ri_o$lower),
                   hi = ifelse(age_int < 12, ri_y$upper, ri_o$upper),
                   outside = result_num < lo | result_num > hi)
  vals <- d$outside
  n <- length(vals)
  boot_rates <- replicate(B_fp, mean(sample(vals, n, replace = TRUE)) * 100)
  tibble(test = "aPTT", ri_source = "Direct",
         fp_mean = mean(boot_rates),
         fp_ci_lo = quantile(boot_rates, 0.025),
         fp_ci_hi = quantile(boot_rates, 0.975))
}
fp_boot_direct <- bind_rows(
  ri_direct |> filter(test %in% c("PT", "Fibrinogen")) |>
    rowwise() |>
    mutate(res = list(boot_fp_ci(all_data_3sd, test, lower, upper, B_fp))) |>
    unnest(res) |>
...
fp_boot_all <- bind_rows(fp_boot_mfr, fp_boot_ref, fp_boot_cont,
                         fp_boot_guven, fp_boot_direct) |>
  mutate(fp_label = sprintf("%.1f (%.1f-%.1f)", fp_mean, fp_ci_lo, fp_ci_hi))

message("\n=== TABLE: Out-of-RI Flagging Rates with 95% Bootstrap CI ===")
fp_boot_all |>
