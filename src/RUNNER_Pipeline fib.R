...
source("src/pipeline/mode_deneme.R")
...
m <- function() {
  ...
  inputData <- read.table(file = "data/coa_results/CSV cont fib 2025.10.17.csv", header = T, sep =";",
                          stringsAsFactors =F)
  ...
  deniz(allRes = allRes, withCI = TRUE, RIperc = RIperc, xlim = c(1,17),xlab = "Yaş",ylab = "PT, saniye",
  ...
  # plotRICurveEstimates(allRes = allRes, withCI = TRUE, RIperc = RIperc)
  deniz(allRes = allRes, withCI = TRUE, RIperc = RIperc, xlim = c(1,17),xlab = "Yaş",ylab = "Fibrinoken, g/L",
        cols =  c( "red",   "white",  "white",  "blue",
                   "white",  "white",  "red")
