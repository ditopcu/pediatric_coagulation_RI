...
# Direct (a posteriori) limits: 2.5th / 97.5th percentiles with 90% CIs,
# nonparametric CLSI EP28-A3c rank method (point estimate rank p*(n+1) with
# linear interpolation; CI ranks from the exact binomial, Reed et al. 1971).
# Computed in manu_revise/direct_ri_final.R from the direct cohort files in
# data/coa_results/, on the same percentile basis as refineR and GAMLSS.
# The aPTT 1-12 y cohort comes from data/coa_results/aptt 1-12 12.08.2026.xlsx,
# adopted 2026-08-13; the file it replaced is kept as direkt aptt 1-12_old.xlsx.
# The two share 150 of their 170 records; the other 20 differ, which moves the
# lower limit and leaves the upper unchanged.
ri_comparison <- tribble(
  ...
  "PT",          "1-18 y",    "Direct",   305,    8.55,   8.41,         8.68,         11.23,  10.90,        12.00,
  "aPTT",        "1-12 y",   "refineR",  10091,  24.30,  23.30,        24.50,        35.30,  34.40,        35.40,
  "aPTT",        "1-12 y",   "Direct",   170,    22.55,  21.90,        23.40,        35.57,  34.60,        35.90,
  "aPTT",        "12-18 y",  "refineR",  6910,   23.30,  22.70,        23.70,        33.90,  33.10,        34.10,
  "aPTT",        "12-18 y",  "Direct",   150,    23.58,  22.90,        24.10,        33.32,  32.60,        34.00,
  "Fibrinogen",  "1-18 y",   "refineR",  3166,   1.89,   1.84,         1.92,         3.79,   3.55,         4.06,
  "Fibrinogen",  "1-18 y",   "Direct",   198,    2.07,   2.04,         2.10,         3.89,   3.56,         4.13
)
...
# Same 2.5th/97.5th direct limits as in ri_comparison above.
ri_direct <- tribble(
  ~test,         ~age_group,  ~lower, ~upper,
  "PT",          "1-18",       8.55,  11.23,
  "aPTT",        "1-12",      22.55,  35.57,
  "aPTT",        "12-18",     23.58,  33.32,
  "Fibrinogen",  "1-18",       2.07,   3.89
)
...
# Bootstrap file lists cover up to NBootstrap = 200 (reviewer revision P1);
# get_continuous_ri() keeps only files that exist, so runs with fewer
# completed iterations still work.
cont_pt   <- get_continuous_ri("cont_out/PT_gamlss_PointEst.RData",
                               paste0("cont_out/PT_gamlssModel_Est_", 1:200, ".RData"))
cont_aptt <- get_continuous_ri("cont_out/aPTT_gamlss_PointEst.RData",
                               paste0("cont_out/aPTT_gamlssModel_Est_", 1:200, ".RData"))
cont_fib  <- get_continuous_ri("cont_out/Fib_gamlss_PointEst.RData",
                               paste0("cont_out/Fib_gamlssModel_Est_", 1:200, ".RData"))
