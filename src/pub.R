...
# Overall RI bands, taken from ri_comparison in common.R so they cannot fall
# behind the limits reported everywhere else. One band per reported partition;
# the age span of each follows the partition label, where "1-12 y" is
# age_year < 12 and the data end at age 17.
age_span <- function(group) {
  switch(group, "1-18 y" = c(1, 17), "1-12 y" = c(1, 11), "12-18 y" = c(12, 17),
         stop("unknown age_group: ", group))
}

make_bands <- function(tname) {
  d <- ri_comparison |> filter(test == tname, method == "refineR")
  stopifnot(nrow(d) >= 1)
  bind_rows(lapply(seq_len(nrow(d)), function(i) {
    r <- d[i, ]; s <- age_span(r$age_group)
    tibble(Percentile = c("0.025", "0.975"),
           age_start  = s[1], age_end = s[2],
           point_est  = c(r$lower,       r$upper),
           ci_lo      = c(r$lower_ci_lo, r$upper_ci_lo),
           ci_hi      = c(r$lower_ci_hi, r$upper_ci_hi))
  })) |>
    mutate(Percentile = factor(Percentile, levels = c("0.975", "0.025")))
}

pt_bands   <- make_bands("PT")
aptt_bands <- make_bands("aPTT")
...
# Shared x-axis range for the two aPTT partitions (reviewer request): with
# scales = "free_x" each aPTT facet would get its own range and the identical
# manufacturer limits would appear at different positions in the two panels.
# Invisible points at the common range force both aPTT facets to one scale;
# PT and fibrinogen facets keep their own free scales.
aptt_xrange <- ri_with_mfr |>
  filter(test == "aPTT") |>
  summarise(xmin = min(lower_ci_lo, lower.y), xmax = max(upper_ci_hi, upper.y))

aptt_blank <- ri_with_mfr |>
  filter(test == "aPTT") |>
  distinct(panel_label) |>
  crossing(x = c(aptt_xrange$xmin, aptt_xrange$xmax)) |>
  mutate(method = "refineR")

fig_3 <- ri_with_mfr |>
  ggplot(aes(y = fct_rev(method), fill = method)) +
  geom_blank(data = aptt_blank, aes(x = x, y = method), inherit.aes = FALSE) +
...
plot_continuous_ri <- function(point_est_file, bs_files, input_data,
  ...
  p <- ggplot(ri_df, aes(x = age)) +
    ...
    annotate("text", x = 18.25, y = last(ri_df$p97.5), label = "97.5th",
             colour = PAL$contin, size = 3, hjust = 0, fontface = "bold") +
    annotate("text", x = 18.25, y = last(ri_df$p50),   label = "50th",
             colour = PAL$contin, size = 3, hjust = 0, fontface = "bold") +
    annotate("text", x = 18.25, y = last(ri_df$p2.5),  label = "2.5th",
             colour = PAL$contin, size = 3, hjust = 0, fontface = "bold") +
...
fig_4_pt <- plot_continuous_ri(
  ...
  bs_files       = paste0("cont_out/PT_gamlssModel_Est_", 1:200, ".RData"),
...
fig_4_aptt <- plot_continuous_ri(
  ...
  bs_files       = paste0("cont_out/aPTT_gamlssModel_Est_", 1:200, ".RData"),
...
fig_4_fib <- plot_continuous_ri(
  ...
  bs_files       = paste0("cont_out/Fib_gamlssModel_Est_", 1:200, ".RData"),
